...
function deleteRow(){var a=event.srcElement.id;alertify.confirm("Potvrda brisanja","Da li želite da uklonite stavku?",function(){$.ajax({type:"POST",url:"deleteRow.php",dataType:"json",data:{stavka:a},success:function(){location.reload()},error:function(){location.reload()}})},function(){}).set('labels', {ok: 'Da',cancel: 'Ne'});;}
...
function checkForm() {
    ...
    if (($odOsOu.length == 0) && ($vrstaSociva.length == 0) && ($indeks.length == 0) && ($materijal.length == 0) && ($dizajn.length == 0) && ($kolicina == 0) && ($mpc.length == 0) && ($broj_radnog_naloga.length == 0)) {
        alertify.alert('Niste unijeli sve potrebne parametre', "Obavezna polja za unos su: - Od/Os/Ou - Vrsta sočiva - Vrsta materijala - Dizajn - Index - Prečnik - Količina - MPC/kom - Broj radnog naloga");
        return false;
    }

    if ($odOsOu.length == 0) {
        document.getElementById('select1').focus();
        alertify.alert('Greška', 'Niste izabrali OD,OS ili OU');
        return false;
    }
...
function checkForm2() {
    ...
    if (($odOsOu.length == 0) && ($vrstaSociva.length == 0) && ($indeks.length == 0) && ($dizajn.length == 0) && ($kolicina == 0) && ($mpc.length == 0) && ($broj_radnog_naloga.length == 0)) {
        alertify.alert('Niste unijeli sve potrebne parametre', "Obavezna polja za unos su: - Od/Os/Ou - Vrsta sočiva - Dizajn - Index - Prečnik - Količina - MPC/kom - Broj radnog naloga");
        return false;
    }
...
function checkLagerBifoProgForm() {
    ...
    if (($odOsOu.length == 0) && ($vrstaSociva.length == 0) && ($materijal == 0) && ($kolicina == 0)) {
        alertify.alert('Niste unijeli sve potrebne parametre', "Obavezna polja za unos su: - Od/Os/Ou - Vrsta sočiva - Vrsta materija - Količina - MPC/kom - Broj radnog naloga");
        return false;
    }
    ...
    if ($id_stavke.length == 0) {
        ...
        $.ajax({
            type: 'POST',
            url: '../poloptic/addRowLagBifoProg.php',
            dataType: 'json',
            data: ({
                stavka: inputVal
            }),
