...
function deleteRow(){var a=event.srcElement.id;alertify.confirm("Potvrda brisanja","Da li želite da uklonite stavku?",function(){$.ajax({type:"POST",url:"deleteRow.php",dataType:"json",data:{stavka:a},success:function(){location.reload()},error:function(){location.reload()}})},function(){}).set('labels', {
            ok: 'Da',
            cancel: 'Ne'
        });;}
...
function checkEmptyTable() {
    ...
    for (let i = 0; i < elems.length; i++) {
        if ($(elems[i]).val().trim() == '') {
            ...
            if (((id.substring(0, 1)) == "m") || ((id.substring(0, 1)) == "r")) {
                isEmpty = 1;
    ...
    if (isEmpty == 1) {
        alertify.alert('Informacija', '<strong> MPC/kom </strong> i <strong>Broj radnog naloga </strong> je obavezno popuniti za sve stavke narudžbenice!');
    }

    if (isEmpty == 0) {
        alertify.confirm("Potvrda slanja", "Da li želite da pošaljete narudžbenicu?",
            function() {
                window.location.href = "mail.php";
            },
            function() {
                alertify.error('Odbijen zahtjev');
            }).set('labels', {
            ok: 'Da',
            cancel: 'Ne'
        });;
