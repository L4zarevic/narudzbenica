...
$(document).keypress(function(e) {if (e.which == 13) {$('input,textarea').attr('autocomplete', 'off');var inputVal = 0;for (var i = 1; i <= 2796; i++) {if (document.getElementById(i).value.length != 0) {if (document.getElementById(i).value > 0) {if (!isNaN(document.getElementById(i).value)) {inputVal = document.getElementById(i).value + "#" + i;add(inputVal);}}}}}});
//Metod za azuriranje reda u tabeli naruzbenice
function getValue(){var e,t;13==event.keyCode&&(t=(e=event.srcElement.id)+"#"+document.getElementById(e).value,$.ajax({type:"POST",url:"updateRow.php",dataType:"json",data:{stavka:t},success:function(){location.reload()},error:function(){location.reload()}}))}
//Metod za brisanje reda u tabeli narudzbenica
function deleteRow(e){e.stopPropagation();var a=event.srcElement.id;alertify.confirm("Potvrda brisanja","Da li želite da uklonite stavku?",function(){$.ajax({type:"POST",url:"deleteRow.php",dataType:"json",data:{stavka:a},success:function(){location.reload()},error:function(){location.reload()}})},function(){}).set('labels', {ok: 'Da',cancel: 'Ne'});;}
//Metod za validaciju forme u unosu specijale
function getMPCValue(){var e,t;13==event.keyCode&&(t=(e=event.srcElement.id)+"#"+document.getElementById(e).value,$.ajax({type:"POST",url:"updateRow.php",dataType:"json",data:{stavka:t},success:function(){location.reload()},error:function(){location.reload()}}))}
...
function checkForm() {
    ...
    $odOsOu = $('#select1').find("option:selected").text();
    $vrstaSociva = $('#select2').find("option:selected").text();
    $materijal = $('#select3').find("option:selected").text();
    $dizajn = $('#select4').find("option:selected").text();
    $visina = $('#select5').find("option:selected").text();
    $segment = document.getElementById("segment").value;
    $pd = document.getElementById("pd").value;
    $baza = $('#select7').find("option:selected").text();
    $indeks = $('#select8').find("option:selected").text();
    $precnik1 = $('#select9').find("option:selected").text();
    $precnik2 = $('#select10').find("option:selected").text();
    $kolicina = document.getElementById("kolicina").value;
    $sph = $('#select11').find("option:selected").text();
    $cyl = $('#select12').find("option:selected").text();
    $ugao = document.getElementById("ugaoCilindra").value;
    $add = $('#select13').find("option:selected").text();
    $jm = $('#select14').find("option:selected").text();
    $kolicina = document.getElementById("kolicina").value;
    $tretman1 = $('#select15').find("option:selected").text();
    $tretman2 = $('#select16').find("option:selected").text();
    ...
    if ($vrstaSociva.length == 0) {
        document.getElementById('select2').focus();
        alertify.alert('Greška', 'Niste izabrali vrstu sočiva');
        return false;
    }
	if ($dizajn.length == 0) {
        document.getElementById('select4').focus();
        alertify.alert('Greška', 'Niste izabrali dizajn');
        return false;
    }
	if ($materijal.length == 0) {
        document.getElementById('select3').focus();
        alertify.alert('Greška', 'Niste izabrali materijal');
        return false;
    }
    if ($indeks.length == 0) {
        document.getElementById('select8').focus();
        alertify.alert('Greška', 'Niste izabrali index');
        return false;
    }
	if (($precnik1.length == 0) && ($precnik2.length == 0)){
	    document.getElementById('select9').focus();
        alertify.alert('Greška', 'Niste izabrali prečnik');
        return false;
    }
    
    if ($vrstaSociva == "Progresiv") {
        if ($visina.length == 0) {
            document.getElementById('select5').focus();
            alertify.alert('Greška', 'Niste izabrali visinu ugradnje');
            return false;
        }
        if ($pd.length == 0) {
            document.getElementById('pd').focus();
            alertify.alert('Greška', 'Niste unijeli pupilarnu distancu');
            return false;
        }
    }
	
	if ((($cyl.length != 0) && ($ugao.length == 0))){
		document.getElementById('ugaoCilindra').focus();
        alertify.alert('Greška', 'Niste unijeli ugao');
        return false;
	}
	
	if ((($vrstaSociva == "Bifokal") || ($vrstaSociva == "Progresiv")) && (($cyl.length != 0) && ($add.length == 0))){
		document.getElementById('select13').focus();
        alertify.alert('Greška', 'Niste izabrali Add/Dig.');
        return false;
	}

    if ($kolicina == 0) {
        document.getElementById('kolicina').focus();
        alertify.alert('Greška', 'Niste unijeli količinu');
        return false;
    }

    if (isNaN($kolicina)) {
        document.getElementById('kolicina').focus();
        alertify.alert('Greška', 'Neispravan unos količine');
        return false;
    }
	
	if ($dizajn != "Standard UC"){
		 if ($tretman1.length == 0) {
            document.getElementById('select15').focus();
            alertify.alert('Greška', 'Niste izabrali tretman');
            return false;
        }
    ...
    if ($mpc.length == 0) {
        ...
        alertify.alert('Informacija', "Nije unesen MPC/kom");
        return false;
    }

    if ($broj_radnog_naloga.length == 0) {
        document.getElementById('broj_radnog_naloga').focus();
        alertify.alert('Informacija', "Nije unesen broj radnog naloga");
        return false;
    }

    $id_stavke = document.getElementById("id_stavke").value;
    var inputVal = "";
    if ($id_stavke.length == 0) {
        inputVal = $odOsOu + "###" + $vrstaSociva + "###" + $dizajn + "###" + $visina + "###" + $segment + "###" + $baza + "###" + $indeks + "###" + $materijal + "###" + $precnik1 + "###" + $precnik2 + "###" + $sph + "###" + $cyl + "###" + $ugao + "###" + $add + "###" + $jm + "###" + $kolicina + "###" + $tretman1 + "###" + $tretman2 + "###" + $pd + "###" + $mpc + "###" + $broj_radnog_naloga + "###" + $napomena;
        $.ajax({
            type: 'POST',
            url: '../poloptic/addRowSpecial.php',
            dataType: 'json',
            data: ({
                stavka: inputVal
            }),
            ...
            error: function() {
                window.location.href = "../poloptic/specijala.php?msg=2";
            }
        });
    } else {
        inputVal = $odOsOu + "###" + $vrstaSociva + "###" + $dizajn + "###" + $visina + "###" + $segment + "###" + $baza + "###" + $indeks + "###" + $materijal + "###" + $precnik1 + "###" + $precnik2 + "###" + $sph + "###" + $cyl + "###" + $ugao + "###" + $add + "###" + $jm + "###" + $kolicina + "###" + $tretman1 + "###" + $tretman2 + "###" + $pd + "###" + $mpc + "###" + $broj_radnog_naloga + "###" + $napomena + "###" + $id_stavke;
        $.ajax({
            type: 'POST',
            url: '../poloptic/updateRowSpecial.php',
            dataType: 'json',
            data: ({
                stavka: inputVal
            }),
            ...
            error: function() {
                window.location.href = "../poloptic/specijala.php?msg=3";
            }
        });
    }
}
...
function checkForm2() {
    ...
    $odOsOu = $('#select1').find("option:selected").text();
    $vrstaSociva = $('#select2').find("option:selected").text();
    $dizajn = document.getElementById("dizajn").value;
    $visina = $('#select5').find("option:selected").text();
    $segment = document.getElementById("segment").value;
    $pd = document.getElementById("pd").value;
    $baza = $('#select7').find("option:selected").text();
    ...
    $jm = $('#select14').find("option:selected").text();
    $kolicina = document.getElementById("kolicina").value;
    $tretman1 = document.getElementById("tretman1").value;
    $tretman2 = document.getElementById("tretman2").value;
    $mpc = document.getElementById("mpc").value;
    $broj_radnog_naloga = document.getElementById("broj_radnog_naloga").value;
    $napomena = document.getElementById("napomena").value;
    ...
    if ($vrstaSociva.length == 0) {
	 ...
	 if ($dizajn.length == 0) {
        document.getElementById('dizajn').focus();
        alertify.alert('Greška', 'Niste unijeli dizajn');
        return false;
    }
    if ($indeks.length == 0) {
        document.getElementById('select8').focus();
    ...
    if ($id_stavke.length == 0) {
        ...
        inputVal = $odOsOu + "###" + $vrstaSociva + "###" + $dizajn + "###" + $visina + "###" + $segment + "###" + $baza + "###" + $indeks + "###" + $precnik1 + "###" + $precnik2 + "###" + $sph + "###" + $cyl + "###" + $ugao + "###" + $add + "###" + $jm + "###" + $kolicina + "###" + $tretman1 + "###" + $tretman2 + "###" + $pd + "###" + $mpc + "###" + $broj_radnog_naloga + "###" + $napomena;

        $.ajax({
            type: 'POST',
            url: 'addRowSpecial.php',
            dataType: 'json',
            data: ({
                stavka: inputVal
            }),
            ...
            error: function() {
                window.location.href = "specijala.php?msg=2";
            }
        });
    } else {
        inputVal = $odOsOu + "###" + $vrstaSociva + "###" + $dizajn + "###" + $visina + "###" + $segment + "###" + $baza + "###" + $indeks + "###" + $precnik1 + "###" + $precnik2 + "###" + $sph + "###" + $cyl + "###" + $ugao + "###" + $add + "###" + $jm + "###" + $kolicina + "###" + $tretman1 + "###" + $tretman2 + "###" + $pd + "###" + $mpc + "###" + $broj_radnog_naloga + "###" + $napomena + "###" + $id_stavke;
        $.ajax({
            type: 'POST',
            url: 'updateRowSpecial.php',
            dataType: 'json',
            data: ({
                stavka: inputVal
            }),
            ...
            error: function() {
                window.location.href = "specijala.php?msg=3";
            }
        });
    }
}

function checkFormMoptic() {
    var $id_stavke = "";
    var $materijal = "";
    var $kolicina = "";
    var $sph = "";
    ...
    $vrsta_materijala = document.getElementById("vrsta_materijala").value;
    $sph = $('#select11').find("option:selected").text();
    $cyl = $('#select12').find("option:selected").text();
    $jm = $('#select14').find("option:selected").text();
    $kolicina = document.getElementById("kolicina").value;
    ...
    $napomena = document.getElementById("napomena").value;
	 ...
	 if ($vrsta_materijala.length == 0) {
        document.getElementById('select4').focus();
        alertify.alert('Greška', 'Niste unijeli vrstu materijala');
        return false;
    }
    ...
    if ($id_stavke.length == 0) {
        ...
        inputVal = $vrsta_materijala + "###" + $sph + "###" + $cyl + "###" + $jm + "###" + $kolicina + "###" + $mpc + "###" + $broj_radnog_naloga + "###" + $napomena;
        ...
        $.ajax({
            ...
            error: function() {
                window.location.href = "lager.php?msg=2";
            }
        });
    } else {
        inputVal = $vrsta_materijala + "###" + $sph + "###" + $cyl + "###" + $jm + "###" + $kolicina + "###" + $mpc + "###" + $broj_radnog_naloga + "###" + $napomena + "###" + $id_stavke;
        $.ajax({
            type: 'POST',
            url: 'updateRowLager.php',
            dataType: 'json',
            data: ({
                stavka: inputVal
            }),
            ...
            error: function() {
                window.location.href = "lager.php?msg=3";
            }
        });
    }
}

function checkFormLenses(){
	var $id_stavke = "";
	var $odOs = "";
	var $period = "";
	var $tip = "";
    ...
    $odOs = document.getElementById("select1").value;
	$period = $('#period').find("option:selected").text();
	$tip = $('#tip').find("option:selected").text();
    $sph = $('#select11').find("option:selected").text();
    $cyl = $('#select12').find("option:selected").text();
	$ugao = document.getElementById("ugao").value;
    $bc = document.getElementById("bc").value;
	$td = document.getElementById("velicina").value;
    $jm = document.getElementById("select14").value;
    $kolicina = document.getElementById("kolicina").value;
    $mpc = document.getElementById("mpc").value;
    $broj_radnog_naloga = document.getElementById("broj_radnog_naloga").value;
    $napomena = document.getElementById("napomena").value;
	...
	if ($bc.length == 0){
        ...
        alertify.alert('Informacija', "Niste unijeli baznu krivinu");
        return false;
	}
	
	if ($td.length == 0){
		document.getElementById('td').focus();
        alertify.alert('Informacija', "Niste unijeli veličinu");
        return false;
	}
	
	if ($kolicina == 0){
        document.getElementById('kolicina').focus();
        alertify.alert('Informacija', 'Niste unijeli količinu');
        return false;
    }
    ...
    if ($id_stavke.length == 0) {
        ...
        inputVal = $odOs + "###" + $period + "###" + $tip + "###" + $sph + "###" + $cyl + "###" + $ugao + "###" + $bc + "###" + $td + "###" + $jm + "###" + $kolicina + "###" + $mpc + "###" + $broj_radnog_naloga + "###" + $napomena;
        $.ajax({
            type: 'POST',
            url: 'addRowLenses.php',
            dataType: 'json',
            data: ({
                stavka: inputVal
            }),
            ...
            error: function() {
                window.location.href = "lenses.php?msg=2";
            }
        });
    } else {
        inputVal = $odOs + "###" + $period + "###" + $tip + "###" + $sph + "###" + $cyl + "###" + $ugao + "###" + $bc + "###" + $td + "###" + $jm + "###" + $kolicina + "###" + $mpc + "###" + $broj_radnog_naloga + "###" + $napomena + "###" + $id_stavke;
        $.ajax({
            type: 'POST',
            url: 'updateRowLenses.php',
            dataType: 'json',
            data: ({
                stavka: inputVal
            }),
            ...
            error: function() {
                window.location.href = "lenses.php?msg=3";
            }
        });
    }
...
var $currentTable;var $row;for(var i=1;i<=8;i++){$(".tabela"+i+" td").on('click', function() {/*clean*/if ($currentTable != null) {$currentTable.find('td').removeClass('selected');$row.removeClass('selected');}$currentTable = $(this).closest('table');$row = $(this).closest('tr');var index = $(this).index();/*select row*/$row.find('td:eq(0)').addClass('selected');/*select column*/$currentTable.find('tr:first-child').each(function() {$(this).find('td').eq(index).addClass('selected');});$currentTable.find('tr:last-child').each(function() {$(this).find('td').eq(index).addClass('selected');});});}

function checkLagerBifoProgForm() {
    ...
    if (isNaN(document.getElementById("kolicina").value)) {
        document.getElementById('kolicina').focus();
        alertify.alert('Greška', 'Neispravan unos količine');
        return false;
    }
    ...
    var inputVal = "";
    if ($id_stavke.length == 0) {
        inputVal = $odOsOu + "###" + $vrstaSociva + "###" + $materijal + "###" + $sph + "###" + $add + "###" + $jm + "###" + $kolicina + "###" + $mpc + "###" + $broj_radnog_naloga + "###" + $napomena;
        $.ajax({
            type: 'POST',
            url: 'addRowLagBifoProg.php',
            dataType: 'json',
            data: ({
                stavka: inputVal
            }),
            ...
            error: function() {
                window.location.href = "lager_bifocal_progresiv.php?msg=2";
            }
        });
    } else {
        inputVal = $odOsOu + "###" + $vrstaSociva + "###" + $materijal + "###" + $sph + "###" + $add + "###" + $jm + "###" + $kolicina + "###" + $mpc + "###" + $broj_radnog_naloga + "###" + $napomena + "###" + $id_stavke;
        $.ajax({
            type: 'POST',
            url: 'updateRowLagBifoProg.php',
            dataType: 'json',
            data: ({
                stavka: inputVal
            }),
            ...
            error: function() {
                window.location.href = "lager_bifocal_progresiv.php?msg=3";
            }
        });
    }
}
...
function checkPartsForm() {
    ...
    if ($brend.length == 0) {
        document.getElementById('brend').focus();
        alertify.alert('Greška', 'Niste unijeli brend');
        return false;
    }
    if ($model.length == 0) {
        document.getElementById('model').focus();
        alertify.alert('Greška', 'Niste unijeli model');
        return false;
    }
    ...
    var inputVal = "";
    if ($id_stavke.length == 0) {
        inputVal = $brend + "###" + $model + "###" + $velicina + "###" + $napomena;
        $.ajax({
            type: 'POST',
            url: 'addRowParts.php',
            dataType: 'json',
            ...
            error: function() {
                window.location.href = "spare_parts.php?msg=2";
            }
        });
    } else {
        inputVal = $brend + "###" + $model + "###" + $velicina + "###" + $napomena + "###" + $id_stavke;
        $.ajax({
            type: 'POST',
            url: 'updateRowParts.php',
            dataType: 'json',
            data: ({
                stavka: inputVal
            }),
            ...
            error: function() {
                window.location.href = "spare_parts.php?msg=3";
            }
        });
    }
}
...
function updateEntireRowPol() {
    ...
    for (var i = 1; i < table.rows.length; i++) {
        ...
        table.rows[i].onclick = function() {
           ...
           var btn = document.getElementById("dugmeNaruci");

            if (this.cells[0].innerHTML == "Spec") {
				document.getElementById("select10").value = "";

                if (window.event.ctrlKey) {
                    document.getElementById("id_stavke").value = "";
					 btn.innerHTML = '<i class="fa fa-save"></i>&nbsp;Sačuvaj stavku';
                }else{
                    document.getElementById("id_stavke").value = this.id;
				    btn.innerHTML = '<i class="fa fa-edit"></i>&nbsp;Izmijeni stavku';
				}

                    document.getElementById("select1").value = this.cells[1].innerHTML;
                    $("#select2 option:contains(" + this.cells[2].innerHTML + ")").prop('selected', 'selected').change();
                    $("#select4 option:contains(" + this.cells[3].innerHTML + ")").prop('selected', 'selected').change();
                    document.getElementById("select5").value = this.cells[4].innerHTML;
                    document.getElementById("segment").value = this.cells[5].innerHTML;
                    document.getElementById("select7").value = this.cells[6].innerHTML;
                    document.getElementById("select8").value = this.cells[7].innerHTML;
                    //$("#select3 option:has(" + this.cells[8].innerHTML + ")").attr('selected','selected' ).change();
					var vrsta_materijala = this.cells[8].innerHTML;
                    $("#select3 option:contains('" + vrsta_materijala + "')").filter(function(i){ return $(this).text() === vrsta_materijala;}).attr("selected", true);



                    var precnik = this.cells[9].innerHTML;
                    if (precnik.indexOf('/') >= 0) {
                        var DtlStr = precnik.split('/');
                        document.getElementById("select9").value = DtlStr[0];
                        document.getElementById("select10").value = DtlStr[1];
                    } else {
                        document.getElementById("select9").value = this.cells[9].innerHTML;
                    }

                    document.getElementById("select11").value = this.cells[10].innerHTML;
                    document.getElementById("select12").value = this.cells[11].innerHTML;
                    document.getElementById("ugaoCilindra").value = this.cells[12].innerHTML;
                    document.getElementById("select13").value = this.cells[13].innerHTML;
                    document.getElementById("select14").value = this.cells[14].innerHTML;
                    document.getElementById("kolicina").value = this.cells[15].innerHTML;
                    $("#select15 option:contains(" + this.cells[16].innerHTML + ")").prop('selected', 'selected').change();
                    $("#select16 option:contains(" + this.cells[17].innerHTML + ")").prop('selected', 'selected').change();
                    document.getElementById("pd").value = this.cells[18].innerHTML;
                    document.getElementById("mpc").value = this.cells[20].innerHTML;
					document.getElementById("broj_radnog_naloga").value = this.cells[21].innerHTML;
					document.getElementById("napomena").value = this.cells[22].innerHTML;
					
					window.scrollTo({top: 0, behavior: 'smooth'});

            }
			
			if (this.cells[0].innerHTML == "Lager" && (this.cells[2].innerHTML == "Bifokal" || this.cells[2].innerHTML == "Progresiv")){
                ...
                if (window.event.ctrlKey) {
				  ...
				  document.getElementById("select1").value = this.cells[1].innerHTML;
				  document.getElementById("select2").value = this.cells[2].innerHTML;
				  document.getElementById("select3").value = this.cells[8].innerHTML;
				  document.getElementById("select11").value = this.cells[10].innerHTML;
				  document.getElementById("select13").value = this.cells[13].innerHTML;
                  document.getElementById("select14").value = this.cells[14].innerHTML;
                  document.getElementById("kolicina").value = this.cells[15].innerHTML
    ...
    for (var i = 1; i < table.rows.length; i++) {
        table.rows[i].onclick = function() {
           ...
           if (window.event.ctrlKey) {
            ...
            document.getElementById("select1").value = this.cells[0].innerHTML;
            $("#select2 option:contains(" + this.cells[1].innerHTML + ")").prop('selected', 'selected').change();
            document.getElementById("dizajn").value = this.cells[2].innerHTML;
            document.getElementById("select5").value = this.cells[3].innerHTML;
            document.getElementById("segment").value = this.cells[4].innerHTML;
            document.getElementById("select7").value = this.cells[5].innerHTML;
            document.getElementById("select8").value = this.cells[6].innerHTML;

            var precnik = this.cells[7].innerHTML;
            if (precnik.indexOf('/') >= 0) {
                var DtlStr = precnik.split('/');
                document.getElementById("select9").value = DtlStr[0];
                document.getElementById("select10").value = DtlStr[1];
            } else {
                document.getElementById("select9").value = this.cells[7].innerHTML;
            }

            document.getElementById("select11").value = this.cells[8].innerHTML;
            document.getElementById("select12").value = this.cells[9].innerHTML;
            document.getElementById("ugaoCilindra").value = this.cells[10].innerHTML;
            document.getElementById("select13").value = this.cells[11].innerHTML;
            document.getElementById("select14").value = this.cells[12].innerHTML;
            document.getElementById("kolicina").value = this.cells[13].innerHTML;
            document.getElementById("tretman1").value = this.cells[14].innerHTML;
            document.getElementById("tretman2").value = this.cells[15].innerHTML;
            document.getElementById("pd").value = this.cells[16].innerHTML;
            document.getElementById("mpc").value = this.cells[18].innerHTML;
			document.getElementById("broj_radnog_naloga").value = this.cells[19].innerHTML;
            document.getElementById("napomena").value = this.cells[20].innerHTML;
...
function updateEntireRowMopticLager() {
    ...
    for (var i = 1; i < table.rows.length; i++) {
        table.rows[i].onclick = function() {
           ...
           if (window.event.ctrlKey) {
            ...
            document.getElementById("vrsta_materijala").value = this.cells[2].innerHTML;
            document.getElementById("select11").value = this.cells[3].innerHTML;
            document.getElementById("select12").value = this.cells[4].innerHTML;
            document.getElementById("select14").value = this.cells[6].innerHTML;
            document.getElementById("kolicina").value = this.cells[7].innerHTML;
            document.getElementById("mpc").value = this.cells[9].innerHTML;
            document.getElementById("broj_radnog_naloga").value = this.cells[10].innerHTML;
            document.getElementById("napomena").value = this.cells[11].innerHTML;	
			window.scrollTo({top: 0, behavior: 'smooth'});
			}
...
function updateEntireRowMopticLagerBifoProg() {
    ...
    for (var i = 1; i < table.rows.length; i++) {
        table.rows[i].onclick = function() {
           ...
           if (window.event.ctrlKey) {
            ...
            document.getElementById("select1").value = this.cells[0].innerHTML;
			document.getElementById("select2").value = this.cells[1].innerHTML;
            document.getElementById("select3").value = this.cells[2].innerHTML;
            document.getElementById("select11").value = this.cells[3].innerHTML;
            document.getElementById("select13").value = this.cells[4].innerHTML;
            document.getElementById("select14").value = this.cells[6].innerHTML;
            document.getElementById("kolicina").value = this.cells[7].innerHTML;
            document.getElementById("mpc").value = this.cells[9].innerHTML;
            document.getElementById("broj_radnog_naloga").value = this.cells[10].innerHTML;
    ...
    for (var i = 1; i < table.rows.length; i++) {
        table.rows[i].onclick = function() {
           ...
           if (window.event.ctrlKey) {
            ...
            document.getElementById("brend").value = this.cells[0].innerHTML;
			document.getElementById("model").value = this.cells[1].innerHTML;
            document.getElementById("velicina").value = this.cells[2].innerHTML;
            document.getElementById("napomena").value = this.cells[3].innerHTML;
			window.scrollTo({top: 0, behavior: 'smooth'});
			}
        };
    }
...
function updateEntireRowLenses() {
    ...
    for (var i = 1; i < table.rows.length; i++) {
        table.rows[i].onclick = function() {
           ...
           if (window.event.ctrlKey) {
            ...
            document.getElementById("select1").value = this.cells[0].innerHTML;
			var period = this.cells[1].innerHTML;
            $("#period option:contains('" + period + "')").filter(function(i){ return $(this).text() === period;}).attr("selected", true).change();
			document.getElementById("lensesType").value = "";
			document.getElementById("lensesType").value = this.cells[2].innerHTML;
	
			document.getElementById("select11").value = this.cells[3].innerHTML;
			document.getElementById("select12").value = this.cells[4].innerHTML;
			document.getElementById("ugao").value = this.cells[5].innerHTML;
			document.getElementById("bc").value = this.cells[6].innerHTML;
			document.getElementById("velicina").value = this.cells[7].innerHTML;
			document.getElementById("select14").value = this.cells[8].innerHTML;			
			document.getElementById("kolicina").value = this.cells[9].innerHTML;
            document.getElementById("mpc").value = this.cells[11].innerHTML;
			document.getElementById("broj_radnog_naloga").value = this.cells[12].innerHTML;
            document.getElementById("napomena").value = this.cells[13].innerHTML;
			window.scrollTo({top: 0, behavior: 'smooth'});
			}
        };
    }
...
function checkEmptyTable() {var elems = $("table input[type=text]");var isEmpty = 0;for (let i = 0; i < elems.length; i++) {if ($(elems[i]).val().trim() == '') {var id = $(elems[i]).attr('id');if (((id.substring(0, 1)) == "m") || ((id.substring(0, 1)) == "r")) {isEmpty = 1;document.getElementById(id).focus();break;}}}if (isEmpty == 1) {alertify.alert('Informacija', '<strong> MPC/kom </strong> i <strong>Broj radnog naloga </strong> je obavezno popuniti za sve stavke narudžbenice!');}if (isEmpty == 0) {alertify.confirm("Potvrda slanja", "Da li želite da pošaljete narudžbenicu?",function() {window.location.href = "mail.php";},function() {alertify.error('Odbijen zahtjev');}).set('labels', {ok: 'Da',cancel: 'Ne'});;}}

function checkSpecialTable(){alertify.confirm("Potvrda slanja", "Da li želite da pošaljete narudžbenicu?",function() {window.location.href = "mail.php";},function() {alertify.error('Odbijen zahtjev');}).set('labels', {ok: 'Da',cancel: 'Ne'});;}

function checkPartsTable(){alertify.confirm("Potvrda slanja", "Da li želite da pošaljete narudžbenicu?",function() {window.location.href = "mail.php";},function() {alertify.error('Odbijen zahtjev');}).set('labels', {ok: 'Da',cancel: 'Ne'});;}
